...
from isaaclab.sensors import TiledCameraCfg  # 必须导入相机配置类
from . import mdp
...
@configclass
class LiftCameraSceneCfg(ObjectTableSceneCfg):
...
class DepthObservationsCfg:
    ...
    class DepthCameraPolicyCfg(ObsGroup):
        ...
        image = ObsTerm(
            func=mdp.image, params={"sensor_cfg": SceneEntityCfg("gripper_camera"), "data_type": "distance_to_image_plane"}
        )
...
@configclass
class LiftCameraEnvCfg(LiftEnvCfg):
    """Configuration for the lift environment with depth camera."""

    scene: LiftCameraSceneCfg = LiftCameraSceneCfg(num_envs=16, env_spacing=20)
